...
function decodePrivateKey(nsec) {
  const { data: privateKey } = nip19_exports.decode(nsec);
  return privateKey;
}
function nsecToNpub(nsec) {
  return nip19_exports.npubEncode(getPublicKey(decodePrivateKey(nsec)));
}
async function decryptDirectMessage({ receiver, sender, content }) {
  const receiverPrivateKey = decodePrivateKey(receiver);
  let result;
  try {
    result = await nip04_exports.decrypt(receiverPrivateKey, sender, content);
  } catch (e) {
    console.error(e);
  }
  return result;
...
async function encryptDirectMessage({ receiver, sender, content }) {
  let result;
  try {
    result = await nip04_exports.encrypt(sender, receiver, content);
  } catch (e) {
    console.error(e);
  }
  return result;
}
async function signEvent2({ event, privateKey }) {
  event.id = getEventHash(event);
  event.sig = getSignature(event, privateKey);
  return event;
}
onmessage = async ({ data }) => {
  ...
  if (data.type === "getPublicKey") {
    postMessage({
      uuid: data.uuid,
      payload: getPublicKey(data.payload)
    });
  }
  ...
  if (data.type === "signEvent") {
    const payload = await signEvent2(data.payload);
    postMessage({
      uuid: data.uuid,
      payload
    });
